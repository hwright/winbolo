...
static unsigned long getaddrbyany(char *sp_name) {
  ...
  if (isdigit(*sp_name)) return inet_addr(sp_name);
  for (i = 0; i < 100; i++) {
...
bool httpCreate() {
  ...
  GetPrivateProfileString("WINBOLO.NET", "Host", "wbn.winbolo.net",
                          wbnHostString, FILENAME_MAX, prefs);
  WritePrivateProfileString("WINBOLO.NET", "Host", wbnHostString,
  ...
  if (httpAddrServer.sin_addr.s_addr == INADDR_NONE) {
    ...
    if (phe == nullptr) {
      returnValue = false;
      fprintf(stderr,
              "Unable to locate WinBolo.net host. Winbolo.net disabled.\n");
#ifdef _WIN32
      WSACleanup();
#endif
    } else {
...
int httpDecodeData(BYTE *buff, int len) {
  ...
  while (count < len) {
    if (buff[count] == '%') {
      count++;
      buff[returnValue] =
          HTTP_HEXTOINT(buff[count]) * 16 + HTTP_HEXTOINT(buff[count + 1]);
      returnValue++;
      count += 2;
    } else {
...
int httpSendData(SOCKET sock, BYTE *message, int len) {
  ...
  if (buff != nullptr) {
    ...
    if (encodeLen != -1) {
      returnValue =
          (int)send(sock, HTTP_SEND_HEADER, (int)strlen(HTTP_SEND_HEADER), 0);
      if (returnValue > 0) {
...
int httpSendMessage(BYTE *message, int len, BYTE *response, int maxSize) {
  ...
  struct sockaddr_in
      ourAddress; /*This structure is to allow us to bind an ip to the socket */
  static unsigned long addressToBind =
      0; /*This is the address we're going to bind, this is the address
            winbolo.net will show */

  if (httpStarted) {
    ourAddress.sin_family = AF_INET;
    ourAddress.sin_port =
        0; /*setting this to zero should have it just grab a ephemeral port*/
    ourAddress.sin_addr.s_addr =
        INADDR_ANY; /* grabs the ip from the default interface */
    /*
      This is were we test for an -addr and then put it into the above variable,
    ...
    */
    ...
    if (sock <= 0) {
      ...
      /* bind the socket here, so that it will use the IP address we've set in
       * the above code when it connects to winbolo.net*/
      ret = bind(sock, (struct sockaddr *)&ourAddress, sizeof(ourAddress));
      if (ret < 0) {
        returnValue = -1;
      } else {
        ret = 0;
        ret = connect(sock, (struct sockaddr *)&httpAddrServer,
                      sizeof(httpAddrServer));
        if (ret == SOCKET_ERROR) {
          returnValue = false;
        }
      }
...
bool httpSendLogFile2(char *fileName, BYTE *key, bool wantFeedback,
                      long fileLength) {
  bool returnValue;
  FILE *fp;
  SOCKET sock;
  ...
  sprintf(
      ...
      boundry);
  length = (long)((long)strlen(contentLength)) + fileLength +
           (long)strlen(boundry) + 7;

  sprintf(header,
          "%s%s HTTP/1.0\r\nContent-Type: multipart/form-data; "
          "boundary=%s\r\nContent-Length: %ld\r\n%s",
          HTTP_POST_HEADER, sKey, boundry, length, hostString);
  sock = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
  returnValue = true;
  if (sock <= 0) {
    returnValue = false;
 ...
 *********************************************************/
void httpSetAltIpAddress(const char *iptoset) { strcpy(altIpAddress, iptoset); }
