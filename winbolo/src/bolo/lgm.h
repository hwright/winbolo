...
/* currently these are set to 16 * 5 which is about 80 world coordinates.
   however, the lgm should be able to return to the tank, from the tanks
   perimeter, so, we're changing this to 8
*/
...
struct lgmObj {
  ...
  WORLD destY;
  BYTE state; /* Present state 0 = Idle, 1 = Going to do work, 2 = returning to
                 tank */
  ...
  bool onTop; /* did the lgm land ontop of a building/base/orpillbox? from a
                 parachute?*/
  BYTE obstructed; /* For brain - 0 = free, 1 = touching wall, 2 = completely
                      stuck */
 ...
 *              to the client
 *********************************************************/
void lgmBackInTank(lgm *lgman, map *mp, pillboxes *pb, bases *bs, tank *tnki,
                   bool sendItems);
