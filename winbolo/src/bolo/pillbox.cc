...
bool pillsExistPos(pillboxes *value, BYTE xValue, BYTE yValue) {
  ...
  returnValue = false;
  count = 0;
  while (!returnValue && count < ((*value)->numPills)) {
...
pillAlliance pillsGetAllianceNum(pillboxes *value, BYTE pillNum) {
  ...
  if ((*value) != nullptr) {
    if ((pillNum) <= ((*value)->numPills)) {
      if ((*value)->item[pillNum].armour == 0 &&
          !(*value)->item[pillNum].inTank) {
        returnValue = pillDead;
      } else if ((*value)->item[pillNum].owner ==
                 playersGetSelf(screenGetPlayers())) {
        if ((*value)->item[pillNum].inTank) {
          returnValue = pillTankGood;
        } else {
          returnValue = pillGood;
        }
      } else if (playersIsAllie(screenGetPlayers(),
                                (*value)->item[pillNum].owner,
                                playersGetSelf(screenGetPlayers()))) {
        if ((*value)->item[pillNum].inTank) {
...
void pillsUpdate(pillboxes *value, map *mp, bases *bs, tank *tnk, shells *shs) {
  ...
  for (count = 0; count < (*value)->numPills; count++) {
    ...
    /* Check to see if it should fire ie Is alive, not allied to the tank and
     * not reloading */
    if (!threadsGetContext()) {
      if ((*value)->item[count].armour > 0 &&
          (!playersIsAllie(screenGetPlayers(), (*value)->item[count].owner,
                           playersGetSelf(screenGetPlayers()))) &&
          (*value)->item[count].reload >= (*value)->item[count].speed &&
          !(*value)->item[count].inTank) {
        /* Check to see if tank is in range */
        if (tankX > ((((*value)->item[count].x) << TANK_SHIFT_MAPSIZE) +
                     MAP_SQUARE_MIDDLE)) {
          diffX = tankX - ((((*value)->item[count].x) << TANK_SHIFT_MAPSIZE) +
                           MAP_SQUARE_MIDDLE);
        } else {
          diffX = ((((*value)->item[count].x) << TANK_SHIFT_MAPSIZE) +
                   MAP_SQUARE_MIDDLE) -
                  tankX;
        }
        if (tankY > ((((*value)->item[count].y) << TANK_SHIFT_MAPSIZE) +
                     MAP_SQUARE_MIDDLE)) {
          diffY = tankY - ((((*value)->item[count].y) << TANK_SHIFT_MAPSIZE) +
                           MAP_SQUARE_MIDDLE);
        } else {
          diffY = ((((*value)->item[count].y) << TANK_SHIFT_MAPSIZE) +
                   MAP_SQUARE_MIDDLE) -
                  tankY;
        }

        if (tankArmour <= TANK_FULL_ARMOUR &&
            (!utilIsTankInTrees(mp, value, bs, tankX, tankY) ||
             (diffX < MIN_TREEHIDE_DIST && diffY < MIN_TREEHIDE_DIST) ||
             tankJustFired(tnk))) {
          if (utilIsItemInRange(x, y, tankX, tankY, PILLBOX_RANGE, &amount)) {
            /* Check for closer tanks here */
            if (!playersIsTankCloser(screenGetPlayers(), x, y,
                                     (*value)->item[count].owner, amount)) {
              /* Fire at player */
              if ((*value)->item[count].justSeen) {
                dir = pillsTargetTank(mp, value, bs, x, y, tankX, tankY,
                                      (TURNTYPE)tankDir, tankSpeed,
                                      (tankIsOnBoat(tnk)));
...
bool pillsIsPillHit(pillboxes *value, BYTE xValue, BYTE yValue) {
  ...
  while (!returnValue && count < ((*value)->numPills)) {
    if (((*value)->item[count].x) == xValue &&
        ((*value)->item[count].y) == yValue &&
        ((*value)->item[count].armour > 0) && !(*value)->item[count].inTank) {
 ...
 *********************************************************/
bool pillsDamagePos(pillboxes *value, BYTE xValue, BYTE yValue, bool wantDamage,
                    bool wantAngry) {
  bool returnValue; /* Value to return */
  ...
  count = 0;
  while (!done && count < ((*value)->numPills)) {
    if (((*value)->item[count].x) == xValue &&
        ((*value)->item[count].y) == yValue &&
        ((*value)->item[count].armour > 0) && !(*value)->item[count].inTank) {
      /* Pillbox has been Hit */
      done = true;
      if (wantDamage && (*value)->item[count].armour > 0) {
...
BYTE pillsGetScreenHealth(pillboxes *value, BYTE xValue, BYTE yValue) {
  ...
  while (!done && count < ((*value)->numPills)) {
    if (((*value)->item[count].x) == xValue &&
        ((*value)->item[count].y) == yValue && !(*value)->item[count].inTank) {
      /* Pillbox has been Hit */
      done = true;
 ...
 *********************************************************/
TURNTYPE pillsTargetTank(map *mp, pillboxes *pb, bases *bs, WORLD xValue,
                         WORLD yValue, WORLD tankX, WORLD tankY, TURNTYPE angle,
                         BYTE speed, bool onBoat) {
  TURNTYPE returnValue; /* Value to return */

  if (speed == 0) {
    returnValue = utilCalcAngle(xValue, yValue, tankX, tankY);
  } else {
    returnValue = pillsTargetTankMove(mp, pb, bs, xValue, yValue, tankX, tankY,
                                      angle, speed, onBoat);
  }
 ...
 *  onBoat - Is the tank on a boat
 *********************************************************/
TURNTYPE pillsTargetTankMove(map *mp, pillboxes *pb, bases *bs, WORLD xValue,
                             WORLD yValue, WORLD tankX, WORLD tankY,
                             TURNTYPE angle, BYTE speed, bool onBoat) {
  TURNTYPE returnValue; /* Value to return */
  ...
  while (!found && count < MAX_AIM_ITERATE) {
    ...
    if (mapGetSpeed(mp, pb, bs, bmx, newbmy, onBoat,
                    playersGetSelf(screenGetPlayers())) > 0 &&
        (!onBoat || (onBoat && !isLand) ||
         (onBoat && isLand && speed >= BOAT_EXIT_SPEED))) {
      tankY = (WORLD)(tankY + tankAddY);
    }

    isLand = mapIsLand(mp, pb, bs, newbmx, bmy);
    if (mapGetSpeed(mp, pb, bs, newbmx, bmy, onBoat,
                    playersGetSelf(screenGetPlayers())) > 0 &&
        (!onBoat || (onBoat && !isLand) ||
         (onBoat && isLand && speed >= BOAT_EXIT_SPEED))) {
...
bool pillsDeadPos(pillboxes *value, BYTE xValue, BYTE yValue) {
  ...
  count = 0;
  while (!returnValue && count < ((*value)->numPills)) {
    /* Do the pill's map coords match what was passed and does it have zero
     * armour and is it not in a tank? */
    if ((((*value)->item[count].x) == xValue) &&
        (((*value)->item[count].y) == yValue) &&
 ...
 *********************************************************/
BYTE pillsGetPillNum(pillboxes *value, BYTE xValue, BYTE yValue,
                     bool careInTank, bool inTank) {
  BYTE returnValue; /* Value to return */
  BYTE count;       /* Looping Variable */
  ...
  while (count < ((*value)->numPills)) {
    if (((*value)->item[count].x) == xValue &&
        ((*value)->item[count].y) == yValue) {
      if (!careInTank || ((*value)->item[count].inTank == inTank)) {
 ...
 *********************************************************/
BYTE pillsSetPillOwner(pillboxes *value, BYTE pillNum, BYTE owner,
                       bool migrate) {
  /* Message stuff */
  ...
  if (pillNum > 0 && pillNum <= (*value)->numPills) {
    ...
    if (returnValue == NEUTRAL && !migrate && owner != NEUTRAL) {
      /* Neutral pill */
      ...
      /* Do nothing */
    } else if (!playersIsAllie(screenGetPlayers(), returnValue, owner) &&
               !migrate) {
      /* Stole pill */
      playersMakeMessageName(screenGetPlayers(), owner, messageStr);
    ...
    (*value)->item[pillNum].owner = owner;
    logAddEvent(log_PillSetOwner, pillNum, owner, migrate, 0, 0, nullptr);
    pillNum++;
    /* Winbolo.Net stuff */
    /* FIXME: Check if in an alliance?????? */
    if (!migrate && owner != NEUTRAL) {
      if (returnValue == NEUTRAL) {
        winbolonetAddEvent(WINBOLO_NET_EVENT_PILL_CAPTURE, threadsGetContext(),
                           owner, WINBOLO_NET_NO_PLAYER);
      } else {
        winbolonetAddEvent(WINBOLO_NET_EVENT_PILL_STEAL, threadsGetContext(),
                           owner, returnValue);
      }
    }
    if (!threadsGetContext()) {
 ...
 *********************************************************/
void pillsGetDamagePos(pillboxes *value, BYTE xValue, BYTE yValue,
                       BYTE amount) {
  BYTE count; /* Looping Variable */
 ...
 *********************************************************/
BYTE pillsNumInRect(pillboxes *value, BYTE leftPos, BYTE rightPos, BYTE top,
                    BYTE bottom) {
  BYTE returnValue; /* Value to return */
  BYTE count;       /* Looping Variable */
  ...
  while (count < ((*value)->numPills)) {
    if ((*value)->item[count].x >= leftPos &&
        (*value)->item[count].x <= rightPos && (*value)->item[count].y >= top &&
        (*value)->item[count].y <= bottom &&
        (!playersIsAllie(screenGetPlayers(), (*value)->item[count].owner,
                         playersGetSelf(screenGetPlayers())))) {
      if ((*value)->item[count].armour > 0) {
        returnValue++;
...
void pillsRepairPos(pillboxes *value, BYTE xValue, BYTE yValue,
  ...
  while (count < ((*value)->numPills)) {
    if (((*value)->item[count].x) == xValue &&
      ...
      (*value)->item[count].armour =
          (*value)->item[count].armour + repairAmount;
...
BYTE pillsGetArmourPos(pillboxes *value, BYTE mx, BYTE my) {
  ...
  count = 0;
  while (count < ((*value)->numPills)) {
    if (((*value)->item[count].x) == mx && ((*value)->item[count].y) == my &&
        !(*value)->item[count].inTank) {
...
bool pillsMoveView(pillboxes *value, BYTE *mx, BYTE *my, int xMove, int yMove) {
  ...
  while (count < (*value)->numPills) {
    if (count != oldPill &&
        (playersIsAllie(screenGetPlayers(), myPlayerNum,
                        (*value)->item[count].owner)) &&
        ((*value)->item[count].armour) > 0 && !(*value)->item[count].inTank) {
      if (((yMove == 0 && xMove < 0 && (*value)->item[count].x < *mx) ||
           (xMove > 0 && (*value)->item[count].x > *mx)) ||
          ((xMove == 0 && yMove < 0 && (*value)->item[count].y < *my) ||
           (yMove > 0 && (*value)->item[count].y > *my))) {
        if (utilIsItemInRange(*mx, *my, (*value)->item[count].x,
                              (*value)->item[count].y, (WORLD)nearest, &dist)) {
          nearest = dist;
          found = count;
          returnValue = true;
        }
...
bool pillsGetNextView(pillboxes *value, BYTE *mx, BYTE *my, bool prev) {
  ...
  if (prev) {
    ...
    if (count == PILL_NOT_FOUND) {
      ...
      count--;
      if ((*value)->item[count].armour > 0 && !(*value)->item[count].inTank) {
        okLoop = true;
        count++;
      } else {
        count = 0;
  ...
  /* Find the next item */
  while (!done && count < ((*value)->numPills)) {
    if ((playersIsAllie(screenGetPlayers(), playNumber,
                        (*value)->item[count].owner)) &&
        ((*value)->item[count].armour) > 0 && !(*value)->item[count].inTank) {
      done = true;
...
void pillsBaseHit(pillboxes *value, BYTE mx, BYTE my, BYTE baseOwner) {
  ...
  for (count = 0; count < (*value)->numPills; count++) {
    ...
    if (xDist >= PILL_BASE_HIT_LEFT && xDist <= PILL_BASE_HIT_RIGHT &&
        yDist >= PILL_BASE_HIT_TOP && yDist <= PILL_BASE_HIT_BOTTOM &&
        (*value)->item[count].owner != NEUTRAL &&
        (playersIsAllie(screenGetPlayers(), baseOwner,
                        (*value)->item[count].owner)) &&
        (*value)->item[count].armour > 0) {
...
void pillsDropSetNeutralOwner(pillboxes *value, BYTE owner) {
  ...
  while (count < ((*value)->numPills)) {
    ...
    if (((*value)->item[count].owner) == owner) {
      ...
      netPNBAdd(screenGetNetPnb(), NPNB_PILL_CAPTURE, count, NEUTRAL,
                (*value)->item[count].x, (*value)->item[count].y, 0);
      /* The tank was carrying pills */
      if ((*value)->item[count].inTank) {
        (*value)->item[count].inTank = false;
        /* Send a message that all the pills in the tank are now dead and belong
         * to a 'neutral' player */
...
void pillsMigrate(pillboxes *value, BYTE oldOwner, BYTE newOwner) {
  ...
  while (count < ((*value)->numPills)) {
    if (((*value)->item[count].owner) == oldOwner) {
      ...
      netMNTAdd(screenGetNetMnt(), NMNT_PILLMIGRATE, count, newOwner,
 ...
 *********************************************************/
void pillsGetBrainPillsInRect(pillboxes *value, BYTE leftPos, BYTE rightPos,
  ...
  */
  ...
  while (count < ((*value)->numPills)) {
    ...
    if (((((*value)->item[count].x) >= leftPos &&
      ...
      screenAddBrainObject(PILLS_BRAIN_OBJECT_TYPE, wx, wy, count,
                           (*value)->item[count].armour, owner);
    }
    count++;
  }
}
...
bool pillsSetView(pillboxes *value, BYTE pillNum, BYTE playerNum) {
  ...
  returnValue = false;
  if (pillNum < ((*value)->numPills)) {
    if (!(*value)->item[pillNum].inTank && (*value)->item[pillNum].armour > 0 &&
        playersIsAllie(screenGetPlayers(), playerNum,
                       (*value)->item[pillNum].owner)) {
      returnValue = true;
    }
  }
 ...
 *********************************************************/
void pillsGetMaxs(pillboxes *value, int *leftPos, int *rightPos, int *top,
                  int *bottom) {
  BYTE count; /* Looping Variable */
...
bool pillsIsInView(pillboxes *value, BYTE playerNum, BYTE mx, BYTE my) {
  ...
  count = 0;
  returnValue = false;
  while (count < (*value)->numPills && !returnValue) {
    if (playersIsAllie(screenGetPlayers(), (*value)->item[count].owner,
                       playerNum)) {
