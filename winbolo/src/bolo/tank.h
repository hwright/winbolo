...
/* the number of times to update the tank slide, this is in world coordinates,
...
<Min210> The speed of a bullet is 32 World units per tick. The top speed of a
tank on a boat is 16 World units per tick. <Min210> the max you found was 64?
<wharf-rat> yes
<Min210> ok
<Min210> I'll keep that in mind
<wharf-rat> so its really 0.25 worlds every other tick

we can get away with no using every other tick becuase we're not using a max of
64, so, this code below seems to work. I made autoslowdown and keyslowdown the
same, becuase they should be the same, the rest seems to behave similiarly to
macbolo based on my testing.
*/
...
/* Minimum distance for seeing tank in trees = 3 map squares or 768 world
 * co-ords */
...
typedef enum {
  ...
  TH_KILL_SMALL, /* The tank has been hit, killsed and isn't carrying a lot of
                    stuff so a small explosion */
  TH_KILL_BIG /* The tank has been hit, killsed and is carrying a lot of stuff
                 so a big explosion */
} tankHit;
 ...
 *********************************************************/
void tankNetTankHit(tank *value, map *mp, pillboxes *pb, bases *bs,
...
void tankMovement(tank *value, map *mp, pillboxes *pb, bases *bs, BYTE bmx,
                  BYTE bmy, tankButton tb);
