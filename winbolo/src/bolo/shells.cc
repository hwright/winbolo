 ...
 *
 *********************************************************/
shells shellsCreate(void) { return nullptr; }
 ...
 *********************************************************/
void shellsAddItem(shells *value, WORLD x, WORLD y, TURNTYPE angle,
                   TURNTYPE len, BYTE owner, bool onBoat) {
  shells q;
  int xAdd;
  int yAdd;
 ...
 *********************************************************/
void shellsUpdate(shells *value, map *mp, pillboxes *pb, bases *bs, tank *tk,
                  BYTE numTanks, bool isServer) {
  ...
  while (NonEmpty(position)) {
    needUpdate = true;
    if (position->shellDead &&
        (position->packSent || netGetType() == netSingle)) {
      needUpdate = false;
      ...
      /* Check for colision */
      if (shellsCalcCollision(mp, pb, tk, bs, &newX, &newY, position->angle,
                              position->owner, position->onBoat, numTanks,
                              isServer)) {
        ...
        while (count < numTanks) {
          lgmDeathCheck(
              screenGetLgmFromPlayerNum(screenGetTankPlayer(&tk[count])), mp,
              pb, bs, newX, newY, position->owner);
          count++;
        }
      ...
      if (isServer && testPos >= MINE_START && testPos <= MINE_END) {
        netMNTAdd(screenGetNetMnt(), NMNT_MINEEXPLOSION, 0,
                  screenGetTankPlayer(tk), bmx, bmy);
      }
      count = 0;
      while (count < numTanks) {
        lgmDeathCheck(
            screenGetLgmFromPlayerNum(screenGetTankPlayer(&tk[count])), mp, pb,
            bs, position->x, position->y, position->owner);
        count++;
      }
      if (position->packSent) {
        shellsDeleteItem(value, &position);
 ...
 *  isServer - true if we are a server
 *********************************************************/
bool shellsCalcCollision(map *mp, pillboxes *pb, tank *tk, bases *bs,
                         WORLD *xValue, WORLD *yValue, TURNTYPE angle,
                         BYTE owner, bool onBoat, BYTE numTanks,
                         bool isServer) {
    ...
    *yValue += MAP_SQUARE_MIDDLE;
    ...
    if (isServer || gameT == netSingle) {
      ...
      if (pillsDamagePos(pb, mapX, mapY, true, true)) {
        ...
        netPNBAdd(screenGetNetPnb(), NPNB_PILL_DEAD, (BYTE)(count - 1),
                  pillsGetPillOwner(pb, count), mapX, mapY, 0);
      } else {
  ...
  if (!returnValue) {
    ...
    while (count < numTanks && !returnValue) {
      if (screenGetTankPlayer(&tk[count]) != owner) {
        th = tankIsTankHit(&(tk[count]), mp, pb, bs, *xValue, *yValue, angle,
                           owner);
        switch (th) {
          case TH_HIT:
            returnValue = true;
            ...
            if (!isServer) {
              if ((mapGetPos(mp, tankGetMX(&(tk[count])),
                             tankGetMY(&(tk[count]))) == DEEP_SEA)) {
                /* We have drowned - They have killed us */
                /*								if
                 * (owner != NEUTRAL) { */
                netMNTAdd(screenGetNetMnt(), NMNT_KILLME,
                          playersGetSelf(screenGetPlayers()),
                          screenGetTankPlayer(&(tk[count])), owner, 0xFF);
                /*								}
                 * else { */
                /* Was player get self for first owner parameter all the time */
                /*						            netMNTAdd(NMNT_KILLME,
                   owner, screenGetTankPlayer(&(tk[count])), 0xFF, 0xFF);
                                                                                 }*/
              }
            }
            break;
          case TH_KILL_SMALL:
            returnValue = true;
            if (!isServer) {
              tkExplosionAddItem(screenGetTankExplosions(), *xValue, *yValue,
                                 angle, TK_EXPLODE_LENGTH, TK_SMALL_EXPLOSION);
              /*							if
               * (owner != NEUTRAL) { */
              netMNTAdd(screenGetNetMnt(), NMNT_KILLME,
                        playersGetSelf(screenGetPlayers()),
          ...
          case TH_KILL_BIG:
            ...
            if (!isServer) {
              tkExplosionAddItem(screenGetTankExplosions(), *xValue, *yValue,
                                 angle, TK_EXPLODE_LENGTH, TK_LARGE_EXPLOSION);
              /*							if
               * (owner != NEUTRAL) { */
              /*								netMNTAdd(NMNT_KILLME,
               * owner, screenGetTankPlayer(&(tk[count])), playersGetSelf(), 0);
               */
              /*							} else {
               */
              netMNTAdd(screenGetNetMnt(), NMNT_KILLME,
        ...
        *yValue += MAP_SQUARE_MIDDLE;
        ...
        if (basesCanHit(bs, mapX, mapY, owner)) {
          if (isServer || gameT == netSingle) {
            ...
            netPNBAdd(screenGetNetPnb(), NPNB_BASE_HIT, 0,
                      screenGetTankPlayer(tk), mapX, mapY, 0);
          }
        ...
        case ROAD:
          if (isMine) {
            ...
            mapSetPos(mp, mapX, mapY,
                      (shellsCheckRoad(mp, pb, bs, mapX, mapY, angle)), false,
                      false);
          }
      }
    }
 ...
 *  angle - The angle the shell is travelling
 *********************************************************/
BYTE shellsCheckRoad(map *mp, pillboxes *pb, bases *bs, BYTE mapX, BYTE mapY,
                     TURNTYPE dir) {
 ...
 *********************************************************/
BYTE shellsNetMake(shells *value, BYTE *buff, BYTE noPlayerNum,
                   bool sentState) {
  BYTE returnValue;  /* Value to return */
  ...
  while (NonEmpty(q)) {
    if (!q->packSent && q->creator != noPlayerNum) {
      ...
      if (screenTankInView(noPlayerNum, (BYTE)(q->x >> TANK_SHIFT_MAPSIZE),
                           (BYTE)(q->y >> TANK_SHIFT_MAPSIZE))) {
 ...
 *********************************************************/
void shellsNetExtract(shells *value, pillboxes *pb, BYTE *buff, BYTE dataLen,
                      bool isServer) {
  ...
  while (pos < dataLen) {
    ...
    if (isServer) {
      if (owner != NEUTRAL) {
        ...
        xAdd *= 2;
        yAdd *= 2;
        // Becuase the pillbox fires from the 'center' of the pill, this doesn't
        // translate perfectly back to the pillbox on the east and south side so
        // we have to add 1 world coordinate to the distance check, so that it
        // properly determines that the pillbox did indeed fire the shells.
        // Ultimately this should be fixed by making winbolo fire a pillbox from
        // the turrets of the pillbox, instead of from the 'center' so this is
        // just a hack to get the code functioning correctly again. This problem
        // was revealed becuase the new rounding code makes the bullets move 1
        // world coordinate more and this moved it just enough that this check
        ...
        /* If a pill exists at the location of ??? and a dead pill does not
         * exist at that same location */
        if ((pillsExistPos(pb, (BYTE)((WORLD)(wx - xAdd) >> M_W_SHIFT_SIZE),
                           (BYTE)((WORLD)(wy - yAdd) >> M_W_SHIFT_SIZE))) &&
            (!pillsDeadPos(pb, (BYTE)((WORLD)(wx - xAdd) >> M_W_SHIFT_SIZE),
                           (BYTE)((WORLD)(wy - yAdd) >> M_W_SHIFT_SIZE)))) {
          shouldAdd = true;
        }
      }
    ...
    if (length > 68 &&
        owner == NEUTRAL) { /* Added length check to stop cheating */
      shouldAdd = false;
    } else if (length > 52 && owner != NEUTRAL) {
      /* FIXME: Check shells fired from a tank are near the tank that fired
       * them, they have sufficent shells etc */
      /* Tank max length */
      shouldAdd = false;
    }
 ...
 *********************************************************/
void shellsGetBrainShellsInRect(shells *value, BYTE leftPos, BYTE rightPos,
  ...
  */
  ...
  while (NonEmpty(position)) {
    ...
    if (mx >= leftPos && mx <= rightPos && my >= topPos && my <= bottomPos) {
      ...
      } else if (playersIsAllie(screenGetPlayers(), playerNum,
                                position->owner)) {
        owner = SHELLS_BRAIN_FRIENDLY;
      } else {
        owner = SHELLS_BRAIN_HOSTILE;
      }
      screenAddBrainObject(SHELLS_BRAIN_OBJECT_TYPE, position->x, position->y,
                           0, utilGet16Dir(position->angle), owner);
    }
    position = ShellsTail(position);
  }
}
