...
void lgmUpdate(lgm *lgman, map *mp, pillboxes *pb, bases *bs, tank *tnk) {
  ...
  if (netGetType() == netSingle || threadsGetContext()) {
    ...
    if ((*lgman)->isDead) {
      ...
      if (pos == BUILDING || pos == HALFBUILDING || pillsExistPos(pb, mx, my) ||
          basesExistPos(bs, mx, my)) {
        (*lgman)->onTop = true;
        // if base is owned by player, then, we're not ontop.
        if (basesAmOwner(bs, (*lgman)->playerNum, mx, my)) {
          (*lgman)->onTop = false;
        }
      } else {
        (*lgman)->onTop = false;
      ...
      } else if (((*lgman)->state == LGM_STATE_RETURN) &&
                 ((*tnk)->deathWait == 0)) {
...
bool lgmCheckNewRequest(lgm *lgman, map *mp, pillboxes *pb, bases *bs,
                        tank *tnk, BYTE mapX, BYTE mapY, BYTE *action,
                        BYTE *pillNum, bool *isMine, BYTE *trees, BYTE *mines,
                        bool perform);
 ...
 *********************************************************/
void lgmAddRequest(lgm *lgman, map *mp, pillboxes *pb, bases *bs, tank *tnk,
  ...
  if ((*lgman)->isDead && tankGetArmour(tnk) <= TANK_FULL_ARMOUR) {
  ...
  } else if (netGetType() == netSingle || threadsGetContext()) {
    ...
    BYTE minesAmount;
    bool ok;
    ok = lgmCheckNewRequest(lgman, mp, pb, bs, tnk, mapX, mapY, &action,
    ...
    if (ok) {
      netMNTAdd(screenGetNetMnt(), NMNT_MANACTION, action, (*lgman)->playerNum,
                mapX, mapY);
      (*lgman)->action = action;
    } else {
 ...
 *  tnk    - Pointer to the tank structure
 *********************************************************/
void lgmTankDied(lgm *lgman) { (*lgman)->nextAction = LGM_IDLE; }
...
bool lgmCheckNewRequest(lgm *lgman, map *mp, pillboxes *pb, bases *bs,
                        tank *tnk, BYTE mapX, BYTE mapY, BYTE *action,
                        BYTE *pillNum, bool *isMine, BYTE *trees,
                        BYTE *minesAmount, bool perform) {
  ...
  *pillNum = LGM_NO_PILL;
  ...
  switch (*action) {
    ...
    case LGM_BOAT_REQUEST:
      if (pos != RIVER || isPill || isBase) {
        proceed = false;
      } else if (!tankGetLgmTrees(tnk, LGM_COST_BOAT, perform)) {
        proceed = false;
      } else {
        *trees = LGM_COST_BOAT;
      }
 ...
 *  action - What the new action is
 *********************************************************/
void lgmNewPrimaryRequest(lgm *lgman, map *mp, pillboxes *pb, bases *bs,
                          tank *tnk, BYTE mapX, BYTE mapY, BYTE action) {
  BYTE pillNum;
  bool isMine;
  BYTE trees;
  ...
  if (lgmCheckNewRequest(lgman, mp, pb, bs, tnk, mapX, mapY, &action, &pillNum,
                         &isMine, &trees, &minesAmount, true)) {
    (*lgman)->numTrees = trees;
    (*lgman)->numMines = minesAmount;
    if (isMine) {
      (*lgman)->state = LGM_STATE_IDLE;
    ...
    if (netGetType() == netUdp || threadsGetContext()) {
      serverNetSendManWorkingMessage(screenGetTankPlayer(tnk), mapX, mapY,
                                     minesAmount, trees, pillNum);
    }
  } else {
    (*lgman)->state = LGM_STATE_IDLE;
    ...
    if (threadsGetContext()) {
      serverNetSendManReturnMessge(screenGetTankPlayer(tnk), trees, minesAmount,
                                   pillNum);
    }
  }
}

bool lgmCheckBlessedSquare(BYTE xValue, BYTE yValue, BYTE action, map *mp,
                           pillboxes *pb, bases *bs, tank *tnk) {
  ...
  switch (action) {
    case LGM_TREE_REQUEST:
      if (pos == BUILDING || pos == HALFBUILDING || pos == RIVER ||
          pos == BOAT || pos == DEEP_SEA || isPill) {
        returnValue = false;
      }
      break;
    case LGM_ROAD_REQUEST:
      if (pos == BUILDING || pos == HALFBUILDING || pos == BOAT ||
          pos == DEEP_SEA || isPill) {
        returnValue = false;
      }
      break;
    case LGM_BUILDING_REQUEST:
      if (pos == BUILDING || pos == RIVER || pos == BOAT || pos == DEEP_SEA ||
          isPill) {
        returnValue = false;
      }
...
void lgmMoveAway(lgm *lgman, map *mp, pillboxes *pb, bases *bs, tank *tnk) {
  ...
  if ((bmx == (*lgman)->blessX && bmy == (*lgman)->blessY) || onBoat) {
  ...
  /* Huh? */
  if ((mapGetManSpeed(mp, pb, bs, bmx, newbmy, (*lgman)->playerNum)) > 0 ||
      onBoat) {
  }
  ...
  if ((mapGetManSpeed(mp, pb, bs, bmx, newbmy, (*lgman)->playerNum)) > 0 ||
    ...
    (*lgman)->y = (WORLD)((*lgman)->y + yAdd);
             ...
             lgmCheckBlessedSquare(bmx, newbmy, (*lgman)->action, mp, pb, bs,
                                   tnk)) {
    (*lgman)->y = (WORLD)((*lgman)->y + yAdd);
  ...
  if ((((mapGetManSpeed(mp, pb, bs, newbmx, newbmy, (*lgman)->playerNum))) >
           0 &&
       xAdd != 0) ||
      onBoat) {
    (*lgman)->x = (WORLD)((*lgman)->x + xAdd);
  } else if (newbmx == (*lgman)->blessX && bmy == (*lgman)->blessY &&
             lgmCheckBlessedSquare(newbmx, newbmy, (*lgman)->action, mp, pb, bs,
                                   tnk)) {
    (*lgman)->x = (WORLD)((*lgman)->x + xAdd);
    //  } else if (newbmx == (*lgman)->blessX && bmy == (*lgman)->blessY) {
    //    (*lgman)->x = (WORLD) ((*lgman)->x + xAdd);
  ...
  if (((*lgman)->x - (*lgman)->destX) >= LGM_MIN_GOAL &&
      ((*lgman)->x - (*lgman)->destX) <= LGM_MAX_GOAL &&
      ((*lgman)->y - (*lgman)->destY) >= LGM_MIN_GOAL &&
      ((*lgman)->y - (*lgman)->destY) <= LGM_MAX_GOAL) {
    /* Arrived */
...
void lgmReturn(lgm *lgman, map *mp, pillboxes *pb, bases *bs, tank *tnk) {
  ...
  if (tankGetArmour(tnk) > TANK_FULL_ARMOUR || newmx == 0 || newmy == 0) {
    speed = 0;
    return;
  } else if ((bmx == (*lgman)->blessX && bmy == (*lgman)->blessY) || onBoat) {
    speed = MAP_MANSPEED_TREFBASE;
  } else {
    speed = mapGetManSpeed(mp, pb, bs, bmx, bmy, (*lgman)->playerNum);
  }
  ...
  if ((mapGetManSpeed(mp, pb, bs, newbmx, newbmy, (*lgman)->playerNum)) > 0 ||
      onBoat) {
    (*lgman)->x = (WORLD)((*lgman)->x + xAdd);
  ...
  if (((*lgman)->x >> TANK_SHIFT_MAPSIZE) != (*lgman)->blessX ||
      ((*lgman)->y >> TANK_SHIFT_MAPSIZE) != (*lgman)->blessY) {
    (*lgman)->blessX = 0;
    (*lgman)->blessY = 0;
  }
  ...
  if ((*lgman)->onTop) {
    /*
      ok we landed ontop of a obstruction, so, we're going to keep moving get
      speed, calculate movement.
    */
    speed = 6;
    /* speed is a magic number currently, once we detemine what the ontop speed
       for the various types of terrain are, this will be adjusted
    */
    utilCalcDistance(&xAdd, &yAdd, angle, speed);
    /* once this algorithm is completed, the below edge numbers need to be
     * defined instead of magic */
    onTopEdgeX = 0;
    onTopEdgeY = 0;
    newmx = (WORLD)((*lgman)->x + xAdd +
                    onTopEdgeX); /* ontopedge x and y are to give an 'edge' to
                                    the obstruction */
    ...
    newbmy = (BYTE)newmy;
    /*
          ok here we're going to have to check for edges, if we see the edge of
      the obstruction, we have to stop this is so we don't go outside of the
      obstruction

      we have to check for edges on the x sides, and the y sides. edges will be
      determined by weither the terrain next to us is a obstructed object also,
      if it is, we will run over onto it also, this will allow us to run around
      ontop of blocks of pills, or bases, so we have to check for any pillbox,
      enemy bases, and buildings or half buildings.

          so, we will need to know first how close we are to the edge, if we're
      within a certain number of world coordinates to the edge of the tile we're
      on, do a check for a tile we can run onto in the neighbouring area.
        */
    /* detect the terrain types, don't check diagonal squares. becuase the lgm
     * can't run ontop of diagonal squares */
    ...
    if (currentTerrain == BUILDING || currentTerrain == HALFBUILDING) {
      ...
      /* do the check to see if pills/bases are beside each other, to allow the
         lgm to run around ontop of pills or bases. if pills exist in the
         current location, then check to see if the new location is also a pill,
         if it is, you can go onto it.
      */
      if (pillsExistPos(pb, bmx, bmy)) {
        if (pillsExistPos(pb, newbmx, bmy)) {
          sameTerrainTypeX = true;
        }
        if (pillsExistPos(pb, bmx, newbmy)) {
          sameTerrainTypeY = true;
        }
      } else {
        /* bases will be a bit more complicated, I will have to check for base
           ownership. can't have tbe lgm running over onto a based owned by the
           player.
        */
        if (basesExistPos(bs, bmx, bmy)) {
          if (basesExistPos(bs, newbmx, bmy)) {
            sameTerrainTypeX =
                !basesAmOwner(bs, (*lgman)->playerNum, newbmx, bmy);
          }
          if (basesExistPos(bs, bmx, newbmy)) {
            sameTerrainTypeY =
                !basesAmOwner(bs, (*lgman)->playerNum, bmx, newbmy);
          }
        } else {
          /* must not be ontop of a building anymore, since its failed all the
           * checks, onTop = false*/
          (*lgman)->onTop = false;
        }
      }
    }
    // if the lgm's old mapx coordinate is still the same, then we can add xAdd
    // and yAdd to the lgm's coordinates) ok so the below works, now we need to
    // modify it, to allow us to run onto tiles that are next to us.
    if (bmx == newbmx || sameTerrainTypeX) {
      (*lgman)->x = (WORLD)((*lgman)->x + xAdd);
    }
    if (bmy == newbmy || sameTerrainTypeY) {
      (*lgman)->y = (WORLD)((*lgman)->y + yAdd);
    }
    speed = 0;
  }
  ...
  /* Check for achieved goal */
  tankGetWorld(tnk, &newmx, &newmy);
  if (((*lgman)->x - newmx) >= LGM_RETURN_MIN_GOAL &&
      ((*lgman)->x - newmx) <= LGM_RETURN_MAX_GOAL &&
      ((*lgman)->y - newmy) >= LGM_RETURN_MIN_GOAL &&
      ((*lgman)->y - newmy) <= LGM_RETURN_MAX_GOAL) {
    ...
    /*    if (threadsGetContext() == true) {
          serverNetSendManReturnMessge(screenGetTankPlayer(tnk),
       (*lgman)->numTrees, (*lgman)->numMines, (*lgman)->numPills);
        } */
    lgmBackInTank(lgman, mp, pb, bs, tnk, true);
    if (!threadsGetContext()) {
  ...
  switch ((*lgman)->action) {
    ...
    case LGM_ROAD_REQUEST:
      ...
      if (terrain != BUILDING && terrain != HALFBUILDING && terrain != BOAT &&
          terrain != DEEP_SEA && !isPill && !isBase) {
        if (isMine) {
      ...
      break;
    case LGM_BUILDING_REQUEST:
      if (terrain != BUILDING && terrain != RIVER && terrain != BOAT &&
          terrain != DEEP_SEA && !isPill && !isBase) {
        if (!isMine) {
 ...
 *              to the client
 *********************************************************/
void lgmBackInTank(lgm *lgman, map *mp, pillboxes *pb, bases *bs, tank *tnk,
                   bool sendItems) {
  BYTE trees;
  BYTE minesAmount;
  BYTE pillNum;
  ...
  if (threadsGetContext()) {
    ...
    if ((*lgman)->nextAction != LGM_IDLE) {
      if (/* FIXME - Removed for lgm problem threadsGetContext() == true || */
          netGetType() == netSingle) {
        lgmNewPrimaryRequest(lgman, mp, pb, bs, tnk, (*lgman)->nextX,
                             (*lgman)->nextY, (*lgman)->nextAction);
      } else {
        /* Network game */
        BYTE pillNum;
        bool isMine;
        ...
        ok = lgmCheckNewRequest(lgman, mp, pb, bs, tnk, (*lgman)->nextX,
                                (*lgman)->nextY, &action, &pillNum, &isMine,
                                &trees, &minesAmount, false);
        (*lgman)->numTrees = trees;
        (*lgman)->numMines = minesAmount;

        if (ok) {
          netMNTAdd(screenGetNetMnt(), NMNT_MANACTION, action,
                    (*lgman)->playerNum, (*lgman)->nextX, (*lgman)->nextY);
          (*lgman)->action = action;
        }
      }
      (*lgman)->nextAction = LGM_IDLE;
 ...
 *********************************************************/
bool lgmOnScreen(lgm *lgman, BYTE leftPos, BYTE rightPos, BYTE top,
                 BYTE bottom) {
  bool returnValue; /* Value to return */
  ...
  if (x >= leftPos && x < rightPos && y >= top && y < bottom &&
      (*lgman)->state != LGM_STATE_IDLE && !(*lgman)->inTank) {
    returnValue = true;
  }
  return returnValue;
}
 ...
 *  py      - Y pixel co ord
 *********************************************************/
void lgmGetScreenCoords(lgm *lgman, BYTE leftPos, BYTE topPos, BYTE *mx,
                        BYTE *my, BYTE *px, BYTE *py, BYTE *frame) {
 ...
 *********************************************************/
void lgmDeathCheck(lgm *lgman, map *mp, pillboxes *pb, bases *bs, WORLD wx,
                   WORLD wy, BYTE owner) {
  ...
  if ((threadsGetContext() || netGetType() == netSingle) && !(*lgman)->isDead &&
      !(*lgman)->inTank) {
    WORLD conv;
    dead = false;
    conv = (*lgman)->x - 1;
    ...
    utilIsItemInRange((*lgman)->x, (*lgman)->y, wx, wy, PILLBOX_RANGE,
                      &distance);
    pos = mapGetPos(mp, mx, my);
    solid = false;
    if (pos == BUILDING || pos == HALFBUILDING || pillsExistPos(pb, mx, my) ||
    ...
    if (!solid && distance <= MAP_SQUARE_MIDDLE) {
      dead = true;
    } else if (solid && lgmX == mx && lgmY == my) {
      dead = true;
    }
    ...
    if (dead) {
      ...
      /* Check for tank in mines (i.e. dead) send builder back to spoke it
       * died*/
      if ((*lgman)->destX <= ((MAP_MINE_EDGE_LEFT + 1) << 8) ||
          (*lgman)->destX >= ((MAP_MINE_EDGE_RIGHT - 1) << 8) ||
          (*lgman)->destY <= ((MAP_MINE_EDGE_TOP + 1) << 8) ||
          (*lgman)->destY >= ((MAP_MINE_EDGE_BOTTOM - 1) << 8)) {
        (*lgman)->destX = (*lgman)->x;
        (*lgman)->destY = (*lgman)->y;
      }
      ...
      /* WinBolo.net it */
      winbolonetAddEvent(WINBOLO_NET_EVENT_LGM_LOST, true, (*lgman)->playerNum,
                         WINBOLO_NET_NO_PLAYER);
      if (owner != NEUTRAL) {
        winbolonetAddEvent(WINBOLO_NET_EVENT_LGM_KILL, true, owner,
                           (*lgman)->playerNum);
      }
      /* Process message */
...
void lgmParchutingIn(lgm *lgman) {
  ...
  /* Check for achieved goal */
  if (((*lgman)->x - (*lgman)->destX) >= -16 &&
      ((*lgman)->x - (*lgman)->destX) <= 16 &&
      ((*lgman)->y - (*lgman)->destY) >= -16 &&
      ((*lgman)->y - (*lgman)->destY) <= 16) {
    /* Arrived at drop off spot. Begin trek back to tank */
    if (threadsGetContext()) {
      netPNBAdd(screenGetNetPnb(), NPNB_LGM_DEAD, true, (*lgman)->playerNum, 0,
                0, 0);
    }
 ...
 *  lgman  - Pointer to the lgm sturcture
 *********************************************************/
BYTE lgmGetFrame(lgm *lgman) { return (*lgman)->frame; }

bool lgmIsOut(lgm *lgman) { return !((*lgman)->inTank); }
 ...
 *********************************************************/
void lgmGetStatus(lgm *lgman, tank *tnk, bool *isOut, bool *isDead,
                  TURNTYPE *angle) {
 ...
 *  lgman  - Pointer to the lgm sturcture
 *********************************************************/
WORLD lgmGetWX(lgm *lgman) { return (*lgman)->x; }
 ...
 *  lgman  - Pointer to the lgm sturcture
 *********************************************************/
WORLD lgmGetWY(lgm *lgman) { return (*lgman)->y; }
...
BYTE lgmGetDir(lgm *lgman, tank *tnk) {
  ...
  if ((*lgman)->state == LGM_STATE_GOING) {
    angle = utilCalcAngle((*lgman)->x, (*lgman)->y, (*lgman)->destX,
                          (*lgman)->destY);
  } else {
    angle =
        utilCalcAngle((*lgman)->x, (*lgman)->y, tankGetMX(tnk), tankGetMY(tnk));
  }
 ...
 *  lgman  - Pointer to the lgm sturcture
 *********************************************************/
BYTE lgmGetBrainObstructed(lgm *lgman) { return (*lgman)->obstructed; }
 ...
 *********************************************************/
void lgmNetBackInTank(lgm *lgman, map *mp, pillboxes *pb, bases *bs, tank *tnk,
                      BYTE numTrees, BYTE numMines, BYTE pillNum) {
  (*lgman)->isDead = false;
  (*lgman)->state = LGM_STATE_IDLE;
  (*lgman)->inTank = true;
  (*lgman)->action = LGM_IDLE;
 ...
 *  pillNum  - Pillbox being carries
 *********************************************************/
void lgmNetManWorking(lgm *lgman, tank *tnk, BYTE mapX, BYTE mapY,
                      BYTE numTrees, BYTE numMines, BYTE pillNum) {
  (*lgman)->isDead = false;
  (*lgman)->state = LGM_STATE_GOING;
  (*lgman)->inTank = false;
