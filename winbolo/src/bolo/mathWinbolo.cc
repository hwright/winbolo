...
SPEEDTYPE mathCollisionPostComponentSpeed(int massA, SPEEDTYPE speedA,
  ...
  return (((speedA * (massA - massB)) + (2 * massB * speedB)) /
          (massA + massB));
}
...
int mathAngleTravelUsingComponents(float speedX, float speedY) {
  ...
  /* This body has movement in both the X and Y plane */
    ...
    if ((speedX < 0) && (speedY < 0)) {
      return (int)(180 + mathConvertRadianToDegree(
                             (float)atan((speedY * -1) / speedX)));
    }
    /* Tank is moving SW */
    else if ((speedX < 0) && (speedY > 0)) {
      return (int)((float)(180 + (float)(mathConvertRadianToDegree(
                                     (float)atan((speedY * -1) / speedX)))));
    }
    /* Tank is moving SE */
    else if ((speedX > 0) && (speedY > 0)) {
      return (
          int)(mathConvertRadianToDegree((float)atan((speedY * -1) / speedX)));
    }
    /* Tank is moving NE */
    else if ((speedX > 0) && (speedY < 0)) {
      return (
 ...
 */
void mathUpdateVectorBody(struct mathVectorBody *vB, SPEEDTYPE speed,
                          WORLD currentX, WORLD currentY, WORLD prevX,
                          WORLD prevY, TURNTYPE angle) {
  // int i;
  ...
  actualTankSpeed =
      mathGetDistanceBetweenTwoPoints(currentX, currentY, prevX, prevY);

  (*vB).speedX_prev = (*vB).speedX;
  (*vB).speedY_prev = (*vB).speedY;
  (*vB).angle_prev = (*vB).angle;

  /* There is no difference between the current and previous coordinates or the
   * previous coordinates aren't set to the map origin, which means the tank is
   * brand new.
   */
  if (((currentX - prevX == 0.0) && (currentY - prevY == 0.0)) ||
      ((prevX == 0) && (prevY == 0))) {
    (*vB).angle = angle;
    (*vB).speedX = 0.0;
    (*vB).speedY = 0.0;
  } else {
    (*vB).angle = (TURNTYPE)mathConvertDegreeToBradian(
        (float)mathAngleTravelUsingComponents((float)currentX - prevX,
