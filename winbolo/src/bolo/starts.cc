 ...
 *  value  - Pointer to the starts structure
 *********************************************************/
BYTE startsGetNumStarts(starts *value) { return (*value)->numStarts; }
 ...
 *********************************************************/
void startsGetStart(starts *value, BYTE *x, BYTE *y, TURNTYPE *dir,
                    BYTE playerNum) {
  BYTE count; /* Looping variable */
  BYTE val;
  bool okTank;
  ...
  while (count < (*value)->numStarts && !found) {
    ...
    okTank = screenCheckTankRange((*value)->item[rnd].x, (*value)->item[rnd].y,
                                  playerNum, 512.0);
    if (okTank) {
      found = true;
      val = rnd;
...
void startsGetRandStart(starts *value, BYTE *x, BYTE *y, TURNTYPE *dir) {
  ...
  if ((*value)->numStarts > 0) {
    ...
    while (!found) {
      ...
      testX += 256;
      testY += 256;
      if (!playersCheckCollision(screenGetPlayers(),
                                 playersGetSelf(screenGetPlayers()), testX,
                                 testY, &dummy1, &dummy2)) {
        found = true;
 ...
 *********************************************************/
void startsGetMaxs(starts *value, int *leftPos, int *rightPos, int *topPos,
                   int *bottomPos) {
  BYTE count; /* Looping Variable */
