...
#ifdef _WIN32
      if ((*value).addr[count].sin_addr.S_un.S_addr == addr &&
          (*value).addr[count].sin_port == port) {
#else
      if ((*value).addr[count].sin_addr.s_addr == addr &&
          (*value).addr[count].sin_port == port) {
#endif
        returnValue = count;
...
bool netPlayersShouldDisconnectPlayer(netPlayers *value, BYTE playerNum) {
  ...
  if ((*value).inUse[playerNum]) {
    if (time(nullptr) - (*value).lastHeard[playerNum] >=
        NET_PLAYERS_INACTIVEREMOVE_TIME) {
      returnValue = true;
    }
  }
