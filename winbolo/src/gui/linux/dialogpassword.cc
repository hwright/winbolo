...
static gboolean dialogPasswordClose(GtkWidget *widget, GdkEventButton *event,
                                    gpointer user_data) {
  char pass[MAP_STR_SIZE]; /* Tracker address   */
  const gchar *str = gtk_entry_get_text(GTK_ENTRY(idc_passwordtxt));
  strncpy(pass, str, MAP_STR_SIZE);

  gameFrontSetGameOptions(pass, gameOpen, FALSE, (aiType)0, 0, 0, TRUE);
...
static gboolean dialogPasswordKey(GtkWidget *widget, GdkEventKey *event,
                                  gpointer user_data) {
...
GtkWidget *dialogPasswordCreate(void) {
  ...
  gtk_object_set_data(GTK_OBJECT(dailogPassword), "dailogPassword",
                      dailogPassword);
  ...
  gtk_object_set_data_full(GTK_OBJECT(dailogPassword), "idc_passwordtxt",
                           idc_passwordtxt, (GtkDestroyNotify)gtk_widget_unref);
  ...
  gtk_signal_connect(GTK_OBJECT(dailogPassword), "delete_event",
                     GTK_SIGNAL_FUNC(dialogPasswordClose), nullptr);
  gtk_signal_connect(GTK_OBJECT(button1), "clicked",
                     GTK_SIGNAL_FUNC(dialogPasswordClose), nullptr);
  gtk_signal_connect(GTK_OBJECT(idc_passwordtxt), "key-press-event",
                     GTK_SIGNAL_FUNC(dialogPasswordKey), nullptr);
