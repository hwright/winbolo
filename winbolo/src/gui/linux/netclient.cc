...
bool netClientCreate(unsigned short port) {
  ...
  if (returnValue == TRUE) {
    ...
    if (myUdpSock == INVALID_SOCKET) {
      returnValue = FALSE;
      MessageBox(NULL, langGetText(STR_NETCLIENTERR_CREATEUDPFAIL),
  ...
  if (returnValue == TRUE) {
    ...
    if (ret != 0) {
      returnValue = FALSE;
      MessageBox(NULL, langGetText(STR_NETCLIENTERR_BINDUDPFAIL),
                 DIALOG_BOX_TITLE, MB_ICONINFORMATION);
    } else {
      /* Get what port we were assigned */
      if (getsockname(myUdpSock, (struct sockaddr *)&name,
                      (socklen_t *)&nameLen) == 0) {
...
void netClientSendUdpLast(BYTE *buff, int len) {
  sendto(myUdpSock, (char *)buff, len, 0, (struct sockaddr *)&addrLast,
         sizeof(addrLast));
}
...
void netClientSendUdpServer(BYTE *buff, int len) {
  sendto(myUdpSock, (char *)buff, len, 0, (struct sockaddr *)&addrServer,
         sizeof(addrServer));
}
...
void netClientGetAddress(char *ip, char *dest) {
  ...
  hd = gethostbyaddr((const void *)&addr.sin_addr, sizeof(struct in_addr),
                     AF_INET);
  if (hd == nullptr) {
    strcpy(dest, ip);
  } else {
...
/* FIXME */
bool netClientUdpPingServer(BYTE *buff, int *len, bool wantCrc,
                            bool addNonReliable) {
  bool returnValue;           /* Value to return */
  ...
  *len = 0;
  while (*len <= 0 && timeOut <= TIME_OUT) {
    *len = recvfrom(myUdpSock, (char *)buff, MAX_UDPPACKET_SIZE, 0,
 ...
 *********************************************************/
bool netClientUdpPing(BYTE *buff, int *len, const char *dest,
  ...
  if (returnValue == TRUE) {
    ...
    timeOut = 0;
    tick = timeGetTime();
    sendto(myUdpSock, (char *)buff, *len, 0, (struct sockaddr *)&addr,
           sizeof(addr));
    *len = 0;
    while (*len <= 0 && timeOut <= TIME_OUT) {
      *len = recvfrom(myUdpSock, (char *)buff, MAX_UDPPACKET_SIZE, 0,
 ...
 *  port - Destination port
 *********************************************************/
void netClientSendUdpNoWait(BYTE *buff, int len, char *dest,
                            unsigned short port) {
  struct sockaddr_in addr; /* Socket to send to */
...
bool netClientSetUseEvents(void) {
  ...
  if (ret < 0) {
    returnValue = FALSE;
    MessageBox(NULL, langGetText(STR_NETCLIENTERR_CHAINFAIL), DIALOG_BOX_TITLE,
               MB_ICONINFORMATION);
  }
  return returnValue;
}
...
void netClientUdpCheck(void) {
  ...
  packetLen = recvfrom(myUdpSock, info, MAX_UDPPACKET_SIZE, 0,
                       (struct sockaddr *)&from, (socklen_t *)&fromlen);
  while (packetLen > 0) {
    /* We have data - Yah! */
    ...
    packetLen = recvfrom(myUdpSock, info, sizeof(info), 0,
                         (struct sockaddr *)&from, (socklen_t *)&fromlen);
  }
}
...
void netClientSendUdpTracker(BYTE *buff, int len) {
  sendto(myUdpSock, (char *)buff, len, 0, (struct sockaddr *)&addrTracker,
         sizeof(addrTracker));
}
...
void gameFinderProcessV1(currentGames *cg, char *buff, int len, char *motd) {
  ...
  if (numGames > 0) {
    ...
    while (count < numGames) {
      ...
      ptr++;
      /* Add it */
      currentGamesAddItem(cg, address, port, mapName, version, players, bases,
                          pills, hiddenMines, game, ai, password);
      count++;
    }
  }
}

void gameFinderProcess(HWND *hWnd, currentGames *cg, char *buff, int len,
                       char *motd) {
  char *ptr;
  char *ptr2;
  char param[256];
  ...
  switch (version) {
    case 1:
      ...
      break;
    default:
      MessageBox(NULL, langGetText(STR_NETCLIENTERR_TRACKERVERSIONFAIL),
                 DIALOG_BOX_TITLE, MB_OK);
  }
}

void gameFinderProcessBroadcast(currentGames *cg, INFO_PACKET *info,
                                struct in_addr *pack) {
  ...
  sprintf(version, "%d.%d%d", info->h.versionMajor, info->h.versionMinor,
          info->h.versionRevision);
  currentGamesAddItem(cg, address, info->gameid.serverport, mapName, version,
                      (BYTE)info->num_players, (BYTE)info->free_bases,
                      (BYTE)info->free_pills, mines, (gameType)info->gametype,
                      (aiType)info->allow_AI, password);
}
 ...
 *********************************************************/
bool netClientFindTrackedGames(HWND *hWnd, currentGames *cg,
  ...
  if (returnValue == TRUE) {
    ...
    if (con.sin_addr.s_addr == INADDR_NONE) {
      ...
      if (phe == nullptr) {
        returnValue = FALSE;
        sprintf(txt, "Status: %s",
                langGetText(STR_NETCLIENTERR_TRACKERDNSFAIL));
        gtk_label_set_text(GTK_LABEL(hWnd), txt);
      } else {
  ...
  if (returnValue == TRUE) {
    ...
    if (ret == SOCKET_ERROR) {
      sprintf(txt, "Status: %s",
              langGetText(STR_NETCLIENTERR_TRACKERCONNECTFAIL));
      gtk_label_set_text(GTK_LABEL(hWnd), txt);
      returnValue = FALSE;
    }
  }
  ...
  if (returnValue == TRUE) {
    ...
    if (ret == SOCKET_ERROR) {
      gtk_label_set_text(GTK_LABEL(hWnd),
                         "Status: Error setting socket to non blocking mode.");
      returnValue = FALSE;
    }
  }
  if (returnValue == TRUE) {
    ...
    if (len == 0) {
      ...
      sprintf(txt, "Status: %s",
              langGetText(STR_NETCLIENT_TRACKERPROCESSRESPONSE));
      gtk_label_set_text(GTK_LABEL(hWnd), txt);
...
bool netClientFindBroadcastGames(HWND *hWnd, currentGames *cg) {
  ...
  if (returnValue == TRUE) {
    ...
    if (ret != 0) {
      returnValue = FALSE;
      gtk_label_set_text(GTK_LABEL(hWnd),
                         "Status: Error setting socket options");
    }
  }
  ...
  if (returnValue == TRUE) {
    ...
    gtk_label_set_text(GTK_LABEL(hWnd), txt);
    len = recvfrom(sock, ptr + len, sizeof(buff) - len, 0,
                   (struct sockaddr *)&last, (socklen_t *)&szlast);
    timeOut = 0;
    tick = timeGetTime();
    while (timeOut <= 10000) {  // TIME_OUT
      ...
      usleep(50);
      timeOut = timeGetTime() - tick;
      len = recvfrom(sock, buff, MAX_UDPPACKET_SIZE, 0,
                     (struct sockaddr *)&from, (socklen_t *)&fromlen);
    }
  }
  if (sock != INVALID_SOCKET) {
