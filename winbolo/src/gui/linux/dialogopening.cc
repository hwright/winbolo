...
static gint opening_configure_event(GtkWidget *window,
...
static gboolean dialogOpeningClose(GtkWidget *widget, GdkEventButton *event,
                                   gpointer user_data) {
...
static void dialogOpeningOK(GtkWidget *widget, gpointer user_data) {
  ...
  } else if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(idc_openlocal)) ==
             TRUE) {
    gameFrontSetDlgState(us, openLan);
  } else if (gtk_toggle_button_get_active(
                 GTK_TOGGLE_BUTTON(idc_openinternet)) == TRUE) {
    gameFrontSetDlgState(us, openInternet);
  } else if (gtk_toggle_button_get_active(
                 GTK_TOGGLE_BUTTON(idc_opentutorial)) == TRUE) {
...
GtkWidget *dialogOpeningCreate(void) {
  ...
  label1 = gtk_label_new(
      "Welcome to LinBolo, the multiplayer tank game.\nPlease choose a game "
  ...
  idc_opentutorial = gtk_radio_button_new_with_label(
      vbox1_group, "Tutorial (Instruction for first-time player)");
  vbox1_group = gtk_radio_button_group(GTK_RADIO_BUTTON(idc_opentutorial));
  gtk_widget_ref(idc_opentutorial);
  gtk_object_set_data_full(GTK_OBJECT(dialogOpening), "idc_opentutorial",
                           idc_opentutorial,
                           (GtkDestroyNotify)gtk_widget_unref);
  gtk_widget_show(idc_opentutorial);
  gtk_box_pack_start(GTK_BOX(vbox1), idc_opentutorial, FALSE, FALSE, 0);

  idc_opensingle = gtk_radio_button_new_with_label(
      vbox1_group, "Practise (Single Player; No Network)");
  vbox1_group = gtk_radio_button_group(GTK_RADIO_BUTTON(idc_opensingle));
  gtk_widget_ref(idc_opensingle);
  gtk_object_set_data_full(GTK_OBJECT(dialogOpening), "idc_opensingle",
                           idc_opensingle, (GtkDestroyNotify)gtk_widget_unref);
  ...
  gtk_object_set_data_full(GTK_OBJECT(dialogOpening), "idc_opentcp",
                           idc_opentcp, (GtkDestroyNotify)gtk_widget_unref);
  gtk_widget_show(idc_opentcp);
  gtk_box_pack_start(GTK_BOX(vbox1), idc_opentcp, FALSE, FALSE, 0);

  idc_openlocal = gtk_radio_button_new_with_label(
      vbox1_group, "Local Network (Broadcast Search)");
  vbox1_group = gtk_radio_button_group(GTK_RADIO_BUTTON(idc_openlocal));
  gtk_widget_ref(idc_openlocal);
  gtk_object_set_data_full(GTK_OBJECT(dialogOpening), "idc_openlocal",
                           idc_openlocal, (GtkDestroyNotify)gtk_widget_unref);
  gtk_widget_show(idc_openlocal);
  gtk_box_pack_start(GTK_BOX(vbox1), idc_openlocal, FALSE, FALSE, 0);

  idc_openinternet =
      gtk_radio_button_new_with_label(vbox1_group, "Internet (Tracker Search)");
  vbox1_group = gtk_radio_button_group(GTK_RADIO_BUTTON(idc_openinternet));
  gtk_widget_ref(idc_openinternet);
  gtk_object_set_data_full(GTK_OBJECT(dialogOpening), "idc_openinternet",
                           idc_openinternet,
                           (GtkDestroyNotify)gtk_widget_unref);
  ...
  gtk_object_set_data_full(GTK_OBJECT(dialogOpening), "idc_openskip",
                           idc_openskip, (GtkDestroyNotify)gtk_widget_unref);
  ...
  gtk_object_set_data_full(GTK_OBJECT(dialogOpening), "idc_openquit",
                           idc_openquit, (GtkDestroyNotify)gtk_widget_unref);
  ...
  gtk_object_set_data_full(GTK_OBJECT(dialogOpening), "idc_openhelp",
                           idc_openhelp, (GtkDestroyNotify)gtk_widget_unref);
  ...
  gtk_object_set_data_full(GTK_OBJECT(dialogOpening), "idc_openwinbolonet",
                           idc_openlanguages,
                           (GtkDestroyNotify)gtk_widget_unref);
  ...
  gtk_signal_connect(GTK_OBJECT(dialogOpening), "delete_event",
                     GTK_SIGNAL_FUNC(dialogOpeningClose), nullptr);
  gtk_signal_connect(GTK_OBJECT(idc_openquit), "clicked",
                     GTK_SIGNAL_FUNC(dialogOpeningQuit), nullptr);
  gtk_signal_connect(GTK_OBJECT(idc_openok), "clicked",
                     GTK_SIGNAL_FUNC(dialogOpeningOK), nullptr);
  gtk_signal_connect(GTK_OBJECT(idc_openhelp), "clicked",
                     GTK_SIGNAL_FUNC(dialogOpeningHelp), nullptr);
  gtk_signal_connect(GTK_OBJECT(idc_openlanguages), "clicked",
                     GTK_SIGNAL_FUNC(dialogOpeningWinbolonet), nullptr);
  gtk_signal_connect(GTK_OBJECT(dialogOpening), "destroy",
                     GTK_SIGNAL_FUNC(dialogOpeningCloseBox), nullptr);
  us = dialogOpening;

  gtk_signal_connect(GTK_OBJECT(us), "configure_event",
                     GTK_SIGNAL_FUNC(opening_configure_event), nullptr);
