...
static gboolean dialogTrackerUseFunc(GtkWidget *widget, GdkEventButton *event,
                                     gpointer user_data) {
...
static gboolean dialogTrackerOK(GtkWidget *widget, GdkEventButton *event,
  ...
  enabled = FALSE;
  if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(dialogTrackerUse)) ==
      TRUE) {
    enabled = TRUE;
  }
...
static gboolean dialogTrackerCancel(GtkWidget *widget, GdkEventButton *event,
                                    gpointer user_data) {
  gtk_grab_remove(dialogTrackerUs);
  gtk_widget_destroy(dialogTrackerUs);
  gtk_main_quit();
...
GtkWidget *dialogTrackerCreate(void) {
  ...
  gtk_table_attach(GTK_TABLE(table1), label1, 0, 1, 0, 1, (GtkAttachOptions)(0),
  ...
  gtk_object_set_data_full(GTK_OBJECT(dialogTracker), "dialogTrackerUse",
                           dialogTrackerUse,
                           (GtkDestroyNotify)gtk_widget_unref);
  ...
  gtk_object_set_data_full(GTK_OBJECT(dialogTracker), "dialogTrackerok",
                           dialogTrackerok, (GtkDestroyNotify)gtk_widget_unref);
  ...
  gtk_object_set_data_full(GTK_OBJECT(dialogTracker), "dialogTrackercancel",
                           dialogTrackercancel,
                           (GtkDestroyNotify)gtk_widget_unref);
  ...
  gtk_signal_connect(GTK_OBJECT(dialogTracker), "delete_event",
                     GTK_SIGNAL_FUNC(dialogTrackerCancel), nullptr);
  gtk_signal_connect(GTK_OBJECT(dialogTrackerok), "clicked",
                     GTK_SIGNAL_FUNC(dialogTrackerOK), nullptr);
  gtk_signal_connect(GTK_OBJECT(dialogTrackercancel), "clicked",
                     GTK_SIGNAL_FUNC(dialogTrackerCancel), nullptr);
  gtk_signal_connect(GTK_OBJECT(dialogTrackerUse), "clicked",
                     GTK_SIGNAL_FUNC(dialogTrackerUseFunc), nullptr);
