...
void serverTransportListenUDP(void) {
  ...
  fromlen = sizeof(from);
  packetLen = recvfrom(sockUdp, info, 512 /* MAX_PACKET_SIZE*/, 0,
                       (struct sockaddr *)&from, &fromlen);
  while (packetLen != SOCKET_ERROR) {
    /* We have data - Yah! */
    memcpy(&addrLast, &from, (size_t)sizeof(from));
    serverNetUDPPacketArrive(info, packetLen, from.sin_addr.s_addr,
                             from.sin_port);
...
void serverTransportSendUDPLast(BYTE *buff, int len, bool wantCrc) {
  ...
  sendto(sockUdp, (char *)buff, len, 0, (struct sockaddr *)&addrLast,
         sizeof(addrLast));
}
...
void serverTransportSendUdpTracker(BYTE *buff, int len) {
  sendto(sockUdp, (char *)buff, len, 0, (struct sockaddr *)&addrTracker,
         sizeof(addrTracker));
}
