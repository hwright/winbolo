...
typedef enum { alarmNone, alarmInterrupt, alarmLock, alarmUnlock } alarmType;
...
static void saveMap(char *line) {
  ...
  if (*ptr == EMPTY_CHAR) {
    ...
    if (!serverCoreSaveMap(ptr)) {
      fprintf(stderr,
              "Sorry, an error occured saving the map. Is the path correct?\n");
    }
  }
}
...
static void printHelp() {
  fprintf(stderr,
          "Help:\n Lock - Locks the server and stops new players from "
          "joining.\n Unlock - Unlocks the server and allows new players to "
          "join.\n savemap <map file> - Save the map file to path and file "
          "<map file>\n Say <text> - Sends this message to all players in the "
          "game unless they have turned off server messages.\n Quit - Exits "
          "the server.\n Info - Provide information about the current game\n "
          "Kick - Kicks a player. Case insensitive, prefix a * for WBN "
...
void processKeys(bool isQuiet) {
  ...
  if (isQuiet == true || isNoInput == true) {
    ...
    while (strncmp(keyBuff, "quit", 4) != 0 && isGameOver == false &&
           serverCoreRunning()) {
...
static void processKeys() {
  ...
  if (isQuiet || isNoInput) {
    while (serverCoreRunning() && !isGameOver) {
    ...
    while (strncmp(keyBuff, "quit", 4) != 0 && !isGameOver &&
           serverCoreRunning()) {
      if (strncmp(keyBuff, "help", 4) == 0) {
        /* Help */
        printHelp();
 ...
 *********************************************************/
#ifdef _WIN32
void CALLBACK serverGameTimer(UINT uID, UINT uMsg, DWORD dwUser, DWORD dw1,
                              DWORD dw2) {
...
Uint32 serverGameTimer(Uint32 interval) {
  ...
  if ((tick - oldTick) > SERVER_TICK_LENGTH) {
    ...
    if (quitOnWinFlag || autoClose ||
        (winbolonetIsRunning() && serverCoreGetActualGameType() != gameOpen)) {
      threadsWaitForMutex();
      if (quitOnWinFlag && !isGameOver) {
        isGameOver = serverCoreCheckGameWin(printGameWinners);
      }
      if (autoClose && !isGameOver) {
      ...
      if (isQuiet == false && isGameOver == true) {
        ...
        if (isLogging == true && winbolonetIsRunning() == true &&
            dontSendLog == false) {
        ...
        if (isLogging == true && key[0] != EMPTY_CHAR && dontSendLog == false) {
          screenServerConsoleMessage(
              (char *)"Uploading log file to winbolo.net");
          httpCreate();
  ...
  if (wbnTime > 100) {
    threadsWaitForMutex();
    winbolonetServerUpdate(serverCoreGetNumPlayers(),
                           serverCoreGetNumNeutralBases(),
                           serverCoreGetNumNeutralPills(), false);
    threadsReleaseMutex();
    wbnTime = 0;
  }
...
static void printArgs() {
#ifdef _WIN32
  fprintf(stderr,
          "Usage:\nWinBoloDS -map <Filename> -port <Port> -gametype <GameType> "
          "-mines <Mines> -ai <AiType> -delay <Delay> -limit <Limit> -tracker "
          "<Tracker> -password <Password>\n\n");
#else
  fprintf(stderr,
          "Usage:\nLinBoloDS -map <Filename> -port <Port> -gametype <GameType> "
          "-mines <Mines> -ai <AiType> -delay <Delay> -limit <Limit> -tracker "
          "<Tracker> -password <Password>\n\n");
#endif
  fprintf(stderr,
          "<Filename>    - Path and file name of the map file to open "
          "(-inbuilt can be used\n");
  fprintf(stderr,
          "                instead of -map to enable inbuilt map Everard "
          "Island)\n");
  fprintf(stderr, "<Port>        - Port to run the server on\n");
  fprintf(stderr,
          "<GameType>    - Specifies the game type: \"Open\" or \"Tournament\" "
          "or \"Strict\"\n");
  fprintf(stderr, "\nOptional\n");
  fprintf(
      stderr,
  ...
  fprintf(stderr,
          "<AiType>      - Specifies allowing brains. Valid values are \"no\" "
          "for\n");
  fprintf(stderr,
          "                disallowing, \"yes\" for allowing and \"yesAdv\" "
          "for giving\n");
  fprintf(stderr,
          "                them an advantage. (disallowed if not specified)\n");
  fprintf(stderr,
          "<Delay>       - Specifies the start delay (in seconds) (none if not "
          "specified)\n");
  fprintf(stderr,
  ...
  fprintf(stderr,
          "-autoclose    - Automatically quit the server when all players have "
          "left\n");
  ...
  fprintf(stderr,
          "-maxplayers   - Specifies the maximum number of players that can be "
          "on this\n");
...
static void processTrackerArg(char *argItem, char *trackerAddr,
...
static bool argExist(int numArgs, char **argv, const char *argname) {
  ...
  returnValue = false;
  count = 0;
  while (!returnValue && count < numArgs) {
...
/** Generates a log file name based on current time and map name and copies to
 * fileName */
static void makeLogFileName(char *fileName) {
  ...
  sprintf(fileName, "%04d%02d%02dt%02d%02d%02d_%s", (1900 + tmt->tm_year),
          (1 + tmt->tm_mon), tmt->tm_mday, tmt->tm_hour, tmt->tm_min,
          tmt->tm_sec, mapName);
...
static bool processArgs(int numArgs, char **argv, char *mapName,
                        unsigned short *port, gameType *game, bool *hiddenMines,
                        aiType *ai, int *srtDelay, long *gmeLen,
                        char *trackerAddr, unsigned short *trackerPort,
                        bool *trackerUse, char *password) {
  bool returnValue; /* Value to return */
  int argNum;
  char temp[255];
  ...
  } else if (argExist(numArgs, argv, "inbuilt")) {
...
int main(int argc, char **argv) {
  ...
  if (!processArgs(argc, argv, mapName, &port, &game, &hiddenMines, &ai,
                   &srtDelay, &gmeLen, trackerAddr, &trackerPort, &trackerUse,
                   pass)) {
    fprintf(stderr, "Error in command line parameters\n");
    printArgs();
    exit(0);
  }
  if (argExist(argc, argv, "quiet")) {
  ...
  if (argExist(argc, argv, "maxplayers")) {
  ...
  if (strcmp(mapName, "-inbuilt") == 0) {
    ...
    char tempPath[MAX_PATH]; /* Temp Path for writing out map from resource to
                                read back in */
    ...
    if (res != NULL) {
      ...
      if (hGlobal != NULL) {
        ...
        if (buff != NULL) {
          serverCoreCreateCompressed(buff, 5097, "Everard Island", game,
                                     hiddenMines, srtDelay, gmeLen);
        }
      }
    }
    ...
    serverCoreCreateCompressed(emap, 5097, "Everard Island", game, hiddenMines,
                               srtDelay, gmeLen);

#endif
  } else {
    if (!serverCoreCreate(mapName, game, hiddenMines, srtDelay, gmeLen)) {
  ...
  if (!serverNetCreate(port, pass, ai, trackerAddr, trackerPort, trackerUse,
                       useAddr, (BYTE)maxPlayers)) {
  ...
  if (!argExist(argc, argv, "nowinbolonet")) {
    char mapName[255];
    serverCoreGetMapName(mapName);
    winbolonetCreateServer(
        mapName, port, (BYTE)game, (BYTE)ai, serverCoreGetAllowHiddenMines(),
        (BYTE)(pass[0] != 0), screenNumBases(), screenNumPills(),
        serverCoreGetNumNeutralBases(), serverCoreGetNumNeutralPills(),
        serverCoreGetNumPlayers(), serverCoreGetTimeGameCreated());
  }
  dontSendLog = argExist(argc, argv, "dontsendlog");

  /* Log file generation */
  if (argExist(argc, argv, "log")) {
    ...
    isLogging =
        serverCoreStartLog(logFileName, ai, maxPlayers, (BYTE)(pass[0] != 0));
  }
  ...
  serverTimerGameID =
      timeSetEvent(SERVER_TICK_LENGTH, 10, serverGameTimer, 0, TIME_PERIODIC);
#else
  oldTick = winboloTimer();
  serverTimerGameID =
      SDL_SetTimer(SERVER_TICK_LENGTH, (SDL_TimerCallback)serverGameTimer);
  ...
  if (isLogging && winbolonetIsRunning() &&
      !argExist(argc, argv, "dontsendlog")) {
    winboloNetGetServerKey(key);
  } else {
    key[0] = EMPTY_CHAR;
  }
