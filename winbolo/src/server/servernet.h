...
typedef struct {
  BOLOHEADER h;
  char password[MAP_STR_SIZE]; /* Test password - Pascal string (first byte is
                                  length)  */
} PASSWORD_PACKET;
...
typedef struct {
  BOLOHEADER h;
  char playerName[PLAYER_NAME_LEN]; /* Test player name - Pascal string (first
                                       byte is length)  */
} PLAYERNAME_PACKET;
/* Player Name check packet */
typedef struct {
...
typedef struct {
  ...
  char playerName[PLAYER_NAME_LEN]; /* New player name - Pascal string (first
                                       byte is length)  */
...
void serverNetSendManWorkingMessage(BYTE playerNum, BYTE mapX, BYTE mapY,
                                    BYTE numMines, BYTE numTrees, BYTE pillNum);
